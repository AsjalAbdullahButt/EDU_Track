...
async function loadCourseMarks() {
  ...
  console.log('[course_marks] Student ID:', studentId);
  console.log('[course_marks] Course ID:', courseId);
  console.log('[course_marks] API available:', typeof API !== 'undefined');

  const container = document.getElementById('assessmentTablesContainer');
  if (!container) {
    console.error('[course_marks] Container not found');
    return;
  }

  // Show loading state
  container.innerHTML = '<p class="empty-state">Loading marks data...</p>';
  ...
  try {
    console.log('[course_marks] Fetching data...');
    console.log('[course_marks] Student ID:', studentId, 'Course ID:', courseId);
    ...
    console.log('[course_marks] Marks response:', JSON.stringify(marksRes));
    console.log('[course_marks] Marks type:', typeof marksRes, 'Is array:', Array.isArray(marksRes));
    console.log('[course_marks] Marks length:', marksRes?.length);
    console.log('[course_marks] Courses response:', coursesRes?.length);
    console.log('[course_marks] All course marks:', allCourseMarksRes?.length);

  if (!coursesRes) {
    console.error('[course_marks] No courses data');
  ...
  if (!course) {
    console.error('[course_marks] Course not found, courseId:', courseId);
    container.innerHTML = '<p class="empty-state">Course not found</p>';
    return;
  }
  
  console.log('[course_marks] Course found:', course.course_name);

  if (!marksRes || (Array.isArray(marksRes) && marksRes.length === 0)) {
    console.error('[course_marks] No marks data. marksRes:', marksRes);
    container.innerHTML = '<p class="empty-state">No marks recorded for this course yet</p>';
    return;
  }
  
  console.log('[course_marks] Processing marks...');
  ...
  container.innerHTML = html + summaryTableHtml;
    ...
    const container = document.getElementById('assessmentTablesContainer');
    if (container) {
      container.innerHTML = '<p class="empty-state">Failed to load marks data. Please try again later.</p>';
    }
    if (window.showToast) window.showToast('Failed to load marks', 'error');
  }
}
