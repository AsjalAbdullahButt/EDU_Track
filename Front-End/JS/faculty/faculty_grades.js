...
async function loadFacultyGrades() {
  ...
  const grades = (gradesRes || []).filter(g => courseIds.includes(g.course_id));
  ...
  table.innerHTML = `<thead><tr><th>Student</th><th>Course</th><th>Marks</th><th>Grade</th></tr></thead><tbody>
    ${grades.map(g => {
      const st = students.find(s => s.student_id === g.student_id);
      const cr = courses.find(c => c.course_id === g.course_id);
      return `<tr><td>${st?.student_name || 'N/A'}</td><td>${cr?.course_name || 'N/A'}</td><td>${g.marks || 0}</td><td>${getGrade(g.marks)}</td></tr>`;
    }).join('')}
  </tbody>`;
